...
class ViCLIP(nn.Module):
    ...
    def __init__(self,  tokenizer=None, pretrain=None, freeze_text=True):
        ...
        # 1. First priority: Check VBench root directory for weights
        # Current file: .../VBench/vbench/third_party/ViCLIP/viclip.py
        current_file_path = os.path.abspath(__file__)
        vbench_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_file_path))))
        local_weights = os.path.join(vbench_root, "ViClip-InternVid-10M-FLT.pth")
        
        if os.path.exists(local_weights):
            pretrain = local_weights
            logger.info(f"Found local pretrained weights at {pretrain}")
        
        # 2. Second priority: Use provided pretrain path or default fallback
